# DUST PROTOCOL — HVH Edition

Браузерный 2D top-down HvH-шутер по мотивам CS:GO / de_dust2. Один файл — `index.html`, чистый HTML5/CSS/JS (Canvas API, без внешних зависимостей). Открывается двойным кликом в любом современном браузере.

## Меню как в CS:GO
Полноэкранное главное меню в духе Panorama: верхний навбар с логотипом и вкладками (Играть / Редактор карт), **карточка-превью выбранной карты**, панель настройки матча, большая зелёная кнопка «Играть», нижняя полоса подсказок. **Закупка как в CS:GO**: заголовок с деньгами, левая колонка категорий 1–5, карточки оружия с **силуэтами** и ценами, нижняя панель с настройками раунда и кнопкой «В бой».

## Редактор карт
Открывается **большой кнопкой «Редактор карт» под «Играть»** или вкладкой в навбаре: кнопка **Dust2 загружает оригинальную карту для редактирования** (со спавнами и плентами) или рисуйте свою с нуля — пол, стены, ящики, простреливаемое дерево, окна-ванвеи, пленты A/B, спавны игрока/врагов/тимейтов. ЛКМ — рисовать, ПКМ — стирать, WASD — камера, колесо — зум. Сохранение в браузер, кнопка «Тест» сразу запускает матч на вашей карте; выбор карты — в «Настройке матча» (de_dust2 / Своя карта). Кемп-споты и точки патруля ботов генерируются автоматически.

## Настройка матча
Перед стартом (и в каждой фазе закупки — **без таймера**, старт по кнопке «В БОЙ» или Enter) выбираются:
- **Режим**: Зачистка или **Закладка бомбы** — плент C4 на A/B (удерживать E), таймер 40с, CT сбегаются снимать (урон сбивает снятие); победа — взрыв или зачистка, поражение — дефьюз/смерть/таймаут
- **Число врагов** (1–8) и **тимейтов** (0–4) — союзные боты воюют на вашей стороне (зелёные, с метками)
- **Стартовая сложность** (Новобранец → Элита)

## Умный ИИ
Боты не бегут тупо в пуш: по плану они занимают **кемп-споты и ван-веи** (окно CT, за mid doors, pit, за машиной на long, ниндзя на B…), держат угол с приседом, при перезарядке и между очередями **отходят за укрытие**, а при потере цели садятся в **засаду** на споте рядом с вами — пушат только агрессивные тиры. В режиме бомбы CT обороняют пленты.
Дополнительно: **разъединение** (не слипаются в кучу, держат дистанцию друг от друга), **трейд-фраг** (подтягиваются поддержать союзника, который ведёт бой, на его цель), **репик-префайр** (после кратковременной потери цели реагируют заметно быстрее — наказывают за перепик).

## HVH: боты тоже читеры
В HVH-режиме (включён по умолчанию) боты играют с полным набором читов, как и вы:
- **Wallhack** — всегда знают, где вы, и давят через всю карту
- **Snap-aimbot + hit chance** — мгновенный доворот с настраиваемой точностью по тирам
- **Anti-aim / desync** — модель бота стоит боком к своей реальной позиции (фейк-хитбокс)
- **Spinbot** — видимая модель бешено крутится
- **Resolver** — на каждую очередь решают, вскрыть ли вашу анти-аим
- **Double tap** — элитные боты досылают второй патрон

Побеждает тот, чей resolver и anti-aim работают лучше.

## Интерфейс — как в CS:GO
Верхняя панель счёта (T оранжевым | таймер | CT синим, живые в командах), здоровье/броня с иконками слева-снизу, крупные патроны с названием оружия справа-снизу, деньги под радаром, цветной килфид по командам, **индикаторы состояний** (DT-зарядка, AA/HS, LAG-чок, DUCK, PEEK, сила броска) — под прицелом или столбиком слева, настраиваются в MISC → Indicators.

## Чит-меню (Del) — в стиле gamesense/skeet
Каждая функция в своей директории: RAGE (Aimbot / Accuracy / Exploits / Other), ANTI-AIM (Angles / Desync / Fake lag), LEGIT (Aimbot / Triggerbot), VISUALS (Player ESP / World / Effects), MISC (Interface / Indicators / Crosshair / Feedback / Game), CONFIG (Configs / Keybinds):
- **RAGE** — Aimbot (silent, FOV, hit chance, autofire, target selection), **Minimum damage: Auto** (по HP цели) или слайдер, Auto wall, Resolver, Double tap, **Backtrack**, Auto stop, **умный Hitbox** (auto сам целится в голову, когда тело не убивает — SSG ваншотит даже в шлем), Safe point
- **ANTI-AIM** (глубоко проработан) — **Angles**: yaw base (backward/static/at targets/**freestanding**/spin), yaw offset −180…180, yaw modifier (**sway** с диапазоном и скоростью), spin speed. **Jitter**: mode (off/offset/center/random) + amount + speed. **Desync**: amount, side (switch/left/right), **fake limit**, **desync on shot**, hide shots, **lean/roll**. **Fake lag**: condition (always/moving/standing) + choked ticks. Боты целятся в фейковую модель (пунктир), резолвер вскрывает реальный хитбокс
- **LEGIT** — плавный aimbot (FOV/smooth), RCS, триггербот с задержкой реакции
- **VISUALS** — ESP (corner/full box, name, HP, weapon, distance, glow, resolved-хитбокс), radar hack, sound ESP, offscreen arrows, weapon drops, **damage numbers**, **grenade prediction** (траектория и радиус), backtrack-точки
- **MISC** — watermark с **анимированным clantag**, динамический keybind list, event log, hitmarker/hit sound/kill sound, прицел + spread circle, auto pistol, **auto buy**
- **SKINS** — цвет/паттерн модели игрока и оружия
- **CONFIG** — 3 слота пресетов (alpha/beta/gamma) + reset; всё автосохраняется в localStorage

## Бинды (как в ските) — настраиваются в MISC → Keybinds
Любую клавишу можно переназначить кликом (клик по кнопке → нажать новую клавишу):
- **X** (держать) — fake duck: хитбокс приседа при полной видимой модели
- **V** (держать) — auto peek: точка возврата, после выстрела затягивает обратно за укрытие
- **C** (держать) — форс minimum damage = 1
- **M** — полноэкранный обзор карты (позиции команды, врагов по правилам радара, бомбы)
- **F / T** — тоггл rage / anti-aim на лету
- **Del** — меню чита (перебиндивается)

## Карта — de_dust2 по официальному радару CS:GO
Сетка 64×60 трассирована с официального оверлея: B-плент с **комнатой окна** (window+дверь — оба простреливаются, окно ван-вей), CT spawn с двором и апроном к шорту, **двойные mid doors + окно CT** над мидом, xbox у входа на катвок, catwalk → лестница шорта → плато → A, CT ramp, A-плент с гусём/дефолтом/машиной, **long A** с машиной, контейнером, **комнатой long doors** и питом, suicide из мида в T, туннели верх/низ с **карманом lower**. Коллауты подписаны на полу (GOOSE, CAR, WINDOW, PIT…). **Радар как в CS**: тёмный квадрат с центром на игроке, буквы плентов, точки команды/врагов, маркер бомбы. Зум колесом мыши. A* навигация ботов.

**Прострелы и ван-веи:**
- Деревянные двери (mid doors, long doors, двери B↔CT) простреливаются любым оружием — классический AWP-банг через mid doors работает
- Окно CT↔верх мида: видно и стреляется насквозь, пройти нельзя — ван-вей для AWP
- У каждого оружия своя пробивная способность (AWP пробивает ящики и тонкие стены), урон падает по прочности поверхности
- **Боты тоже простреливают**: потеряв вас из виду за дверью/ящиком, банят по свежей позиции

## Остальное
- 10 видов оружия (включая **SSG 08** и автоснайпу **SCAR-20**) + 4 гранаты с **молотовом** (зона огня 7с с уроном в секунду, боты выбегают из огня); бросок с замахом — держите ЛКМ
- Боты-кемперы **пикают** из-за угла и прячутся обратно, **шьют стены очередями по мин-дамагу** (кемперы на ван-веях — по wallhack-инфе), кидают гранаты и молотовы по вашей последней позиции
- Обычная экономика и в HVH: пистолетный раунд → закуп; у ботов свой бюджет по раундам и HVH-мета (скаут на эко, AWP/автоснайпа на фулл-бае)
- Резолвер общий для rage и legit: триггербот и легит-аим тоже вскрывают desync ботов; без silent aim прицел видимо снапается на цель
- Раундовая система как в CS: счёт T:CT, матч до 8 побед; смерть не завершает раунд, пока живы тимейты или тикает бомба (спектатор за командой)
- Урон оружия по таблицам CS:GO (USP 35, Deagle 53, AK 36, M4 33, SSG 88, AWP 115, SCAR-20 80, Nova 26×8)
- Double tap с зарядкой 3.5с и шансом осечки — у игрока и у ботов одинаково; у ботов авто-вол и равный шанс хедшота
- Пробитие стен (auto wall) со штрафом урона по прочности поверхности
- Синтезированный звук (Web Audio API), частицы, hitmarker, kill/hit sounds
- Стабильные 60 FPS, фиксированный шаг симуляции

## Оружие, отдача и прицел
- **ПКМ-прицеливание только у снайперок** (AWP / SSG 08 / SCAR-20) — у них scope: зум камеры и почти нулевой разброс. У остального оружия ПКМ ничего не даёт (как в CS)
- **Отдача-спрей** у автоматов без прицела: пули по очереди уводит по паттерну (AK/M4/MP7 climb+змейка) — держать зажатым неточно, надо тапать/бёрстить; RCS частично гасит. Прицел растёт с очередью, сбрасывается после паузы
- **R8 Револьвер** ($600) в пистолетах: **задержка перед выстрелом** (0.4с — крутит барабан, как в CS) с **буфером ввода** (клик никогда не теряется — гарантированно один выстрел за нажатие), бьёт 74 в тело / хедшот-машина, высокое пробитие; **у R8 нет double tap**. Есть у ботов и в килфиде
- **Auto revolver** (RAGE → Exploits): сам держит курок R8, чтобы он постоянно стрелял
- **Experimental (only-you)** — экспериментальные фичи только для игрока: **Rapid fire** (×4 темп — и полуавтоматы стреляют при удержании), **No spread** (нулевой разброс), **Fake peek** (держать H — выносит фейк-модель вперёд как приманку), **Defensive lag** (форс-чок и широкий десинк при **отжатии клавиши double tap**)
- **Double tap на клавише** (по умолчанию **Space**): держишь — DT активен и досылает второй патрон; отпустил — срабатывает Defensive lag. Клавиша биндится в CONFIG → Keybinds

## Управление
WASD — движение · мышь — прицел · **ПКМ — прицел (только снайперки)** · Shift — тихий шаг · Ctrl — присед · R — перезарядка · 1–4 — слоты · G — смена гранаты · E — подобрать оружие · B — закупка · **Del — чит-меню**
